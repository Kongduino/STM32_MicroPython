...
class SX127x:
    ...
    # 3. IRQ pins, to be triggered by RFM96W's DIO0~5 pins. These pins each have 2 functions:
    #   3.1 set_handler_for_irq_on_rising_edge()
    #   3.2 detach_irq()
    # 4. a function to blink on-board LED.

    def __init__(self, spi, pin_ss, name = 'SX127x',
                 parameters = {'frequency' : 433000000, 'tx_power_level': 20, 'signal_bandwidth': 125E3/2,
    ...
    def setFrequency(self, frequency):
        ...
        #FSTEP = (FXOSC / 524288)
        # 61.03516
        fq = int((frequency<<9)/31250)
        f0 = (fq >> 16) & 0xFF
        f1 = (fq >> 8) & 0xFF
        f2 = fq & 0xFF
        self.writeRegister(REG_FRF_MSB, f0)
        self.writeRegister(REG_FRF_MID, f1)
        self.writeRegister(REG_FRF_LSB, f2)
        ...
        if sbw < 10:
            ...
            for i in range(len(bins)):
                if sbw <= bins[i]:
                    bw = int(i)
                    break
        self.writeRegister(REG_MODEM_CONFIG_1, int((self.readRegister(REG_MODEM_CONFIG_1) & 0x0f) | (bw << 4)))
    ...
    def writeRegister(self, address, value):
        self.transfer(self.pin_ss, bytes([address | 0x80]), bytes([value]))
    ...
    def setupLoRandom(self):
        self.sleep()
        self.modemconf0 = self.readRegister(REG_OP_MODE)
        time.sleep(0.01)
        self.modemconf1 = self.readRegister(REG_MODEM_CONFIG_1)
        time.sleep(0.01)
        self.modemconf2 = self.readRegister(REG_MODEM_CONFIG_2)
        time.sleep(0.01)
        self.writeRegister(REG_OP_MODE, 0b10001101)
        time.sleep(0.01)
        self.writeRegister(REG_MODEM_CONFIG_1, 0b01110010)
        time.sleep(0.01)
        self.writeRegister(REG_MODEM_CONFIG_2, 0b01101000)
        time.sleep(0.01)
        self.rndMode = True
        self.standby()

    def resetLoRa(self):
        if self.rndMode == True:
            self.sleep()
            self.writeRegister(REG_OP_MODE, self.modemconf0)
            time.sleep(0.01)
            self.writeRegister(REG_MODEM_CONFIG_1, self.modemconf1)
            time.sleep(0.01)
            self.writeRegister(REG_MODEM_CONFIG_2, self.modemconf2)
            time.sleep(0.01)
            self.rndMode = False
            self.standby()

    def getLoRandomByte(self):
        x = 0
        for j in range (0, 8):
            b = self.readRegister(REG_RSSI_WIDEBAND) & 0b00000001
            while (b == self.readRegister(REG_RSSI_WIDEBAND) & 0b00000001):
                # von Neumann extractor.
                self.readRegister(REG_RSSI_WIDEBAND)
            x = (x << 1) | b
        return x
